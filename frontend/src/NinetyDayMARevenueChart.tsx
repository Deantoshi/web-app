...
interface MARevenueData {
  day: string;
  total_90_days_ma_revenue: number;
...
const NinetyDayMARevenueChart: React.FC = () => {
      ...
      return (
        <div className="custom-tooltip" style={{ backgroundColor: '#404040', padding: '10px', border: '1px solid #ccc', borderRadius: '4px' }}>
          ...
          {payload.map((pld: any, index: number) => {
            ...
            if (displayName !== 'Total 90 Days MA Revenue') {
              displayName = displayName.replace('90 Days MA Revenue', '').trim();
            }
            return (
  ...
  const CustomLegend = () => {
    const allKeys = ['total_90_days_ma_revenue', ...deployments];
    return (
      <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexWrap: 'wrap', justifyContent: 'center' }}>
        {allKeys.map((key, index) => (
          ...
          >
            ...
            />
            {
            key === 'total_90_days_ma_revenue' 
            ? 'Total' 
            : capitalize(key.replace('_90_days_ma_revenue', ''))
            }
          </li>
        ))}
      </ul>
  ...
  const visibleData = useMemo(() => {
    return chartData.filter(entry => {
      return Object.entries(visibleLines).some(([key, isVisible]) => {
        return isVisible && entry[`${key}_90_days_ma_revenue`] !== undefined;
      });
    });
    ...
    <div style={{ width: '100%', height: 400 }}>
      <ResponsiveContainer>
        ...
        >
          ...
          <Line 
            type="monotone"
            dataKey="total_90_days_ma_revenue"
            name="Total 90 Days MA Revenue"
            stroke={colors[0]}
            dot={false}
            strokeWidth={2}
            hide={!visibleLines['total_90_days_ma_revenue']}
          />
          {deployments.map((deployment, index) => (
            <Line 
              key={deployment}
              type="monotone"
              dataKey={`${deployment}_90_days_ma_revenue`}
              name={`${deployment}`}
...
export default NinetyDayMARevenueChart;